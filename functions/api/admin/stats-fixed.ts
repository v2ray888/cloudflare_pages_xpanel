export const onRequestGet = async ({ env }) => {
  try {
    ...
    // 获取今日收入
    const todayRevenueResult = await env.DB.prepare(
    ...
    // 获取今日订单数
    const todayOrdersResult = await env.DB.prepare(
    ...
    // 获取兑换码统计
    const totalRedemptionCodesResult = await env.DB.prepare(
      'SELECT COUNT(*) as count FROM redemption_codes'
    ).first();

    const usedRedemptionCodesResult = await env.DB.prepare(
      'SELECT COUNT(*) as count FROM redemption_codes WHERE status = 1'
    ).first();

    // 获取推广统计
    const totalReferralsResult = await env.DB.prepare(
      'SELECT COUNT(*) as count FROM users WHERE referrer_id IS NOT NULL'
    ).first();

    // 获取总佣金
    let totalCommissionsResult = { total: 0 };
    try {
      totalCommissionsResult = await env.DB.prepare(
        'SELECT SUM(commission_amount) as total FROM referral_commissions WHERE status = 1'
      ).first();
    } catch (commissionError) {
      console.error('Commission query error:', commissionError);
      totalCommissionsResult = { total: 0 };
    }
