// Test script to verify generation without prefix
const testNoPrefix = async () => {
  ...
  try {
    ...
    console.log('\nTesting redemption code generation without prefix...');
    
    // Test data for generating redemption codes without prefix
    const testData = {
      plan_id: 1,
      quantity: 3,
    ...
    if (redemptionResponse.ok && redemptionResult.success) {
      console.log('✅ Redemption code generation without prefix test PASSED');
      ...
      if (redemptionResult.data && redemptionResult.data.codes && Array.isArray(redemptionResult.data.codes)) {
        ...
        redemptionResult.data.codes.forEach((code, index) => {
          ...
          // 验证没有前缀
          if (code.length === 16) {
            console.log(`     ✅ No prefix applied (16 characters)`);
          } else {
            console.log(`     ⚠️  Unexpected length: ${code.length}`);
          }
        });
      } else {
